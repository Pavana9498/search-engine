...
class Ui_Dialog(object):
    ...
    def url_key(self, urlString):
        ...
        #     # print("manipulated link")
        #     url = spl[0]+'s:'+spl[1]
        # if url and self.is_in_domain(url):
        #     # print("entered final")
        #     return url
    ...
    def text_urls_from_html(self, conn):
        ...
        a_tag_text = [tag.text for tag in a_tags_content]
        title_tag_content = soup.findAll('title')
        title_tag_content = [tag.text for tag in title_tag_content]
        content = ' '.join(p_tags_content) + ' ' + ' '.join(span_tags_content) + ' ' + ' '.join(h_tags_content) + ' ' + ' '.join(a_tag_text) +' ' + ' '.join(title_tag_content)
        ...
        for a_tag in a_tags_content:
            if 'href' in a_tag.attrs:
                ...
                if parsed_url.hostname is not None and base_domain in parsed_url.hostname and self.url_key(
                    ...
                    if(url_extracted not in out_links):
